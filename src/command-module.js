...
export const RemoveProductImageCommand = function(elem, config) {
...
export const RemoveProductFromDetailCommand = function(element, config) {
  return new Command(removeProductFromDetail, element, config);
}

export const ShowCurrentImageCommand = function(element) {
  return new Command(showCurrentImage, element);
}

export const RemoveProductCommand = function(element, config) {
...
function removeProductImage(element, config) {
  ...
  const file_name = $(element).attr('data-image');
  ...
  $(document).on('click', '.btn-confirmation', function(p) {
    if ($(this).attr('data-answer') === 'yes') {
      ...
      let url = base_url("produk/hapus_gambar_produk"); // URL target
      let ajaxConfig = config.ajaxConfig;
      let ajaxModule = config.ajaxModule;
...
function removeProduct(element, config) {
  const ConfirmationDialog = config.confirmationDialog;
  const idProduct = $(element).attr('data-id-produk');
  ...
  $(document).on('click', '.btn-confirmation', function(p) {
    if ($(this).attr('data-answer') === 'yes') {
      const page = getGETURLParameter("page");
      const per_page = getGETURLParameter("per_page");
      const id_subkategori_input = getGETURLParameter("id_subkategori_input");
      const search_product_name = getGETURLParameter("search_product_name");
      const data = {
        'idProduk': idProduct
      };
...
function showCurrentImage(element) {
  const target = $(element).attr('data-image-preview-target');
  if (!$(target).hasClass('img-list-box-active')) {
    $('.img-list-box').removeClass('img-list-box-active');
    $(`#${target}`).attr('src', $(element).find('img').attr('src'));
