...
async function ajaxInputBefore() {
  ...
  $(this.targetEvent.target).attr('disabled', 'disabled');
  $('.input-error').text('');
...
async function ajaxInputComplete() {
  ...
  $('input[type=submit], button[type=submit]').removeAttr('disabled', 'disabled');
  $(this.targetEvent.target).removeAttr('disabled', 'disabled');
}
...
async function ajaxDeleteSuccess(data) {
  ...
  if (typeof data === 'object') {
    if (!data.notifyStatusError) {
      if (data.hasOwnProperty('notifySuccess')) {
        notify('success', data.notifySuccess);
        // Hapus konten jika berhasil
        if (this.hasOwnProperty("clickedElement")) {
          const contentRemove = $(this.clickedElement).attr("data-target-remove");
          $(this.clickedElement).parents(`.${contentRemove}`).fadeOut('slow', function() {
            $(this).remove();
          })
          // $(`.${this.clickedElement}`).fadeOut('slow', function() {
          //   $(this).remove();
          // });
        }
...
async function ajaxGetJSONRemoveProductDone(data) {
  console.log(data);
  if (!data.notifyStatusError) {
    if (data.hasOwnProperty("notifySuccess")) {
      notify("success", data.notifySuccess);
      if (data.hasOwnProperty("notifyPagination") && data.hasOwnProperty("notifyContent")) {
        $('#paginationContainer').html(data.notifyPagination);
        $('#productContainer').append(data.notifyContent);

      }
      if (data.hasOwnProperty('notifyTotalRowsFoundText')) {
        $(".total-rows-found").html(data.notifyTotalRowsFoundText);
      }
    }
  }
}

async function ajaxGetJSONRemoveProductFail(jqxhr, textStatus, error) {
